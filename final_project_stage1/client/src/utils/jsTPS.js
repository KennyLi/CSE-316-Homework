...
export class EditSubregion_Transaction extends jsTPS_Transaction {
	constructor(listID, field, prev, update, callback) {
		super();
		this.listID = listID;
		this.field = field;
		this.prev = prev;
	...
	async doTransaction() {
		const { data } = await this.updateFunction({ 
				variables:{  _id: this.listID, field: this.field, value: this.update 
						  }
			});
		return data;
    }
    ...
    async undoTransaction() {
		...
		const { data } = await this.updateFunction({ 
				variables:{ _id: this.listID, field: this.field, value: this.prev
						  }
			});
...
export class UpdateMapSubregions_Transaction extends jsTPS_Transaction {
    ...
    async doTransaction() {
        ...
        this.opcode === 0 ? { data } = await this.deleteFunction({
						  ...
						  : { data } = await this.addFunction({
							variables: {subregion: this.item, _id: this.listID, index: this.index}})  
		if(this.opcode !== 0) {
            this.item._id = this.itemID = data.addSubregion;
		}
		return data;
    }
